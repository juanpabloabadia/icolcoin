...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ICOL);
    ...
    if (!settings.contains("nAnonymizeIcolcoinAmount"))
        settings.setValue("nAnonymizeIcolcoinAmount", 1000);

    nAnonymizeIcolcoinAmount = settings.value("nAnonymizeIcolcoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeIcolcoinAmount"))
        SoftSetArg("-anonymizeicolcoinamount", settings.value("nAnonymizeIcolcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in icolcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIcolcoinAmount:
            return QVariant(nAnonymizeIcolcoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIcolcoinAmount:
            nAnonymizeIcolcoinAmount = value.toInt();
            settings.setValue("nAnonymizeIcolcoinAmount", nAnonymizeIcolcoinAmount);
            emit anonymizeIcolcoinAmountChanged(nAnonymizeIcolcoinAmount);
            break;
        case CoinControlFeatures:
