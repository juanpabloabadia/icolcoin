...
CzICOLWallet::CzICOLWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zicol seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZICOLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZICOLSeed_deprecated()) {
                    LogPrintf("%s: Updated zICOL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zicol seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zicol wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzICOLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZICOLCount(nCountLastUsed);
    else if (!walletdb.ReadZICOLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzICOLWallet::Lock()
{
    seedMaster = 0;
}

void CzICOLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzICOLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZICOL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzICOLWallet::LoadMintPoolFromDB()
{
...
void CzICOLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzICOLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzICOLWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zicolTracker->HasPubcoinHash(pMint.first)) {
...
bool CzICOLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZICOL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zicolTracker which also adds to database
    pwalletMain->zicolTracker->Add(dMint, true);
...
void CzICOLWallet::SeedToZICOL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzICOLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzICOLWallet::GenerateDeterministicZICOL(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzICOLWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzICOLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
