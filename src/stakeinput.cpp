...
// The zICOL block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zICOL is a hash of the serial
...
{
    //Create an output returning the zICOL that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZICOLOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zICOL output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zICOL", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZICOLOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zICOL output", __func__);
...
{
    CzICOLTracker* zicolTracker = pwallet->zicolTracker.get();
    CMintMeta meta;
    if (!zicolTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zicolTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a ICOL stake is the outpoint
