...
#ifndef Icolcoin_ZICOLWALLET_H
#define Icolcoin_ZICOLWALLET_H
...
public:
    CzICOLWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZICOL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZICOL(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Icolcoin_ZICOLWALLET_H
