...
public:
    ...
    bool WriteZICOLSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZICOLSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZICOLSeed_deprecated(uint256& seed);
    bool EraseZICOLSeed();
    bool EraseZICOLSeed_deprecated();

    bool WriteZICOLCount(const uint32_t& nCount);
    bool ReadZICOLCount(uint32_t& nCount);
