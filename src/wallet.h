...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ICOL at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zICOL send
enum ZerocoinSpendStatus {
    ZICOL_SPEND_OKAY = 0,                            // No error
    ZICOL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZICOL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZICOL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZICOL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZICOL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZICOL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZICOL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZICOL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZICOL_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZICOL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZICOL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZICOL_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZICOL_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZICOL_SPENT_USED_ZICOL = 14,                      // Coin has already been spend
    ZICOL_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZICOL_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZICOLOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzicolAuto) const;
    ...
    */
    ...
    CzICOLWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzICOLTracker> zicolTracker;
    ...
    void setZWallet(CzICOLWallet* zwallet)
    {
        zwalletMain = zwallet;
        zicolTracker = std::unique_ptr<CzICOLTracker>(new CzICOLTracker(strWalletFile));
    }

    CzICOLWallet* getZWallet() { return zwalletMain; }
    ...
    /** zICOL reset */
    boost::signals2::signal<void()> NotifyzICOLReset;
