...
CzICOLTracker::CzICOLTracker(std::string strWalletFile)
{
...
CzICOLTracker::~CzICOLTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzICOLTracker::Init()
{
...
bool CzICOLTracker::Archive(CMintMeta& meta)
{
...
bool CzICOLTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzICOLTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzICOLTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzICOLTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzICOLTracker::GetSerialHashes()
{
...
CAmount CzICOLTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzICOLTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzICOLTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzICOLTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzICOLTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzICOLTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzICOLTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzICOLTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzICOLTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzICOLTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzICOLTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzICOLTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzICOLTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzICOLTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzICOLTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzICOLTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzICOLTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzicol from DB\n", __func__, listDeterministicDB.size());
    }
